...
private struct EXOClient {
    ...
    private func topology() async throws -> [String] {
        let response = try await request(path: "/state/topology")
    ...
    private func placement() async throws -> EXOInstance {
        ...
        _ = try await request(path: "/instance", method: "POST", body: createBody)
        return EXOInstance(id: instanceID, runnerIDs: runnerIDs)
    }

    private func waitUntilReady(_ instance: EXOInstance) async throws {
        let deadline = Date().addingTimeInterval(TimeInterval(configuration.startupTimeoutSeconds))
        while Date() < deadline {
            let response = try await request(path: "/state")
    ...
    private func remove(_ instance: EXOInstance) async {
        _ = try? await request(path: "/instance/\(instance.id)", method: "DELETE")
    }
    ...
    func infer(prompt: String) async throws -> EXOInferenceResult {
        guard !prompt.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty,
              prompt.utf8.count <= 48_000 else {
            throw OS1Error.message("EXO prompt is invalid")
        }
        _ = try await topology()
        let instance = try await placement()
        do {
            try await waitUntilReady(instance)
            let started = Date()
            let output = try await chat(prompt: prompt)
...
func executeEXO(prompt: String, config: RuntimeConfig) async throws -> EXOInferenceResult {
    try await EXOClient(configuration: try EXOConfiguration(runtimeConfig: config)).infer(prompt: prompt)
}
