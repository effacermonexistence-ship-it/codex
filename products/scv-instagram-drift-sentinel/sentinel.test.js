...
test('accepts the exact healthy v129 release while preserving operational alerts', () => {
  const result = evaluateEndpoint(target, 200, healthyBody())
  assert.equal(result.ok, true)
  assert.equal(result.operational_alert_count, 1)
})

test('accepts the exact separated golden and v129 snapshot control', async () => {
  const catalog = {
  ...
  const latest = {
    ...
    seal: { key: `${prefix}/SCV_TIMESTAMPED_SNAPSHOT_CATALOG_SEAL.json`, sha256: '48f20b24802201400a5108ee9e9d278f6e567511079d47504b6afdfb411e7201' },
